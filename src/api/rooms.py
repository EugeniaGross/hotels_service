...
async def get_rooms(
    ...
    if date_from > date_to:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Дата выезда должна быть позже даты заезда"
        )
...
async def get_room(db: DBDep, hotel_id: int, room_id: int):
    rooms = await db.rooms.get_one_or_none_with_rel(id=room_id, hotel_id=hotel_id)
    if rooms is None:
...
async def delete_room(db: DBDep, hotel_id: int, room_id: int) -> None:
    hotel = await db.rooms.delete(id=room_id, hotel_id=hotel_id)
    await db.commit()
    if hotel is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Номер не найден")
    return
...
async def partial_update_room(
    ...
    room = await db.rooms.edit(_data, exclude_unset=True, id=room_id)
